...
export interface TemplateValueMemberInferred {
	node: TS.Identifier
...
export function inferTemplateValueMember(
	part: TemplatePart,
	...
	if (part.type === TemplatePartType.Property && piece.type === TemplatePartPieceType.AttrValue) {
		return getComponentPropertyMember(template, part, node, analyzer)
	}

	else if (part.type === TemplatePartType.Binding && piece.type === TemplatePartPieceType.AttrValue) {
		return getBindingParameterMember(template, part, node, analyzer)
	}
...
function getComponentPropertyMember(
	template: Template,
	part: TemplatePart,
	node: TS.Identifier,
	analyzer: WorkSpaceAnalyzer
...
export function getBindingParameterMember(
	template: Template,
	part: TemplatePart,
	node: TS.Identifier,
	analyzer: WorkSpaceAnalyzer
